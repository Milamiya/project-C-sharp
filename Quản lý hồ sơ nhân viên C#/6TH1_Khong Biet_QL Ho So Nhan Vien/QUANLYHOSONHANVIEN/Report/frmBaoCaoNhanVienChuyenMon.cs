...
{
    public partial class frmBaoCaoNhanVienChuyenMon : DevComponents.DotNetBar.Office2007Form
    {
        public frmBaoCaoNhanVienChuyenMon()
        {
            InitializeComponent();
        }
        string maCM;
        CRBaoCaoChuyenMon rpt = new CRBaoCaoChuyenMon();
        public frmBaoCaoNhanVienChuyenMon(string str)
            : this()
        {
            maCM = str;
        }

        private void frmBaoCaoNhanVienChuyenMon_Load(object sender, EventArgs e)
        {
            ThongTinCongTyInfo congty = ThamSo.LayTTCongTy();
            Connect();
            rpt.SetParameterValue("paraMaCM", maCM);
