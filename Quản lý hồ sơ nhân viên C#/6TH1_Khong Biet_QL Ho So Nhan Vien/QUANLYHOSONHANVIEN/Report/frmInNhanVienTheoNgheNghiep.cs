...
{
    public partial class frmInNhanVienTheoNgheNghiep : DevComponents.DotNetBar.Office2007Form
    {
        public frmInNhanVienTheoNgheNghiep()
        {
            InitializeComponent();
        }
        string maNN;
        CTBaoCaoTheoNgheNghiep rpt = new CTBaoCaoTheoNgheNghiep();
        public frmInNhanVienTheoNgheNghiep(string str)
            : this()
        {
            maNN = str;
        }

        private void frmInNhanVienTheoNgheNghiep_Load(object sender, EventArgs e)
        {
            ThongTinCongTyInfo congty = ThamSo.LayTTCongTy();
            Connect();
            rpt.SetParameterValue("paraMaNN", maNN);
