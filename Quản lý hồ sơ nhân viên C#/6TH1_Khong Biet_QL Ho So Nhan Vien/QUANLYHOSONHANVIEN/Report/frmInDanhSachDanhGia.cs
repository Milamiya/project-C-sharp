...
{
    public partial class frmInDanhSachDanhGia : DevComponents.DotNetBar.Office2007Form
    {
        public frmInDanhSachDanhGia()
        {
            InitializeComponent();
        }
         string maDDG;
         CRDSDotDanhGia rpt = new CRDSDotDanhGia();
        public frmInDanhSachDanhGia(string str)
            : this()
        {
            maDDG = str;
        }

        private void frmInDanhSachDanhGia_Load(object sender, EventArgs e)
        {
            ThongTinCongTyInfo congty = ThamSo.LayTTCongTy();
            Connect();
            rpt.SetParameterValue("paraMaDDG", maDDG);
